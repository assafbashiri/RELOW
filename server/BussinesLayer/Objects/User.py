...
class User():
    ...
    def add_to_history_purches(self, offer_to_add):
        ...
        self.history_purches.add(offer_to_add.offer_id, offer_to_add)
    ...
    def remove_from_liked_offers(self, offer_to_remove):
        ...
        self.liked_offers.remove(offer_to_remove.offer_id, offer_to_remove)

    def add_to_item_sold(self, offer_sold):
        if self.is_logged is False:
            raise Exception("User Is Not Logged In")
        # check if the offer valid
        self.item_sold.add(offer_sold.offer_id, offer_sold)

    def add_to_active_offers(self, offer_to_add):
        if self.is_logged is False:
            raise Exception("User Is Not Logged In")
        # check if the offer valid
        self.active_offers.add(offer_to_add.offer_id, offer_to_add)

    def remove_from_active_offers(self, offer_to_remove):
        if self.is_logged is False:
            raise Exception("User Is Not Logged In")
        if offer_to_remove not in self.active_offers:
            raise Exception("offer didnt exist in 'Active Offers'")
        self.active_offers.remove(offer_to_remove.offer_id, offer_to_remove)
